...
class Counter:
    ...
    def set_current(self, start):
        if self.min_value <= start <= self.max_value:
            ...
            raise ValueError("Начальное значение должно быть в пределах минимального и максимального значения.")

    def set_max(self, max_max):
        if max_max >= self.min_value:
            self.max_value = max_max
        else:
            raise ValueError("Максимальное значение должно быть больше или равно минимальному значению.")
